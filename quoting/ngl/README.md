...
- **Lifetime benefit period is discontinued** — NGL no longer offers it, so it's
  removed from the tool's UI. The `lifetime` factor remains in `rates.json` only so
  any previously saved lifetime quote still renders; no new quote can select it.
...
- **Spousal / marital discount** — for a married applicant applying **alone** (spouse
  not applying). Implemented as `marital_discount=True` (× 0.95), off by default, via
  the `MARITAL_DISCOUNT` constant at the top of `rater.py`. **The 5% is Blaine's
  recollection and has NOT been verified against the NGL rate notes** — confirm the
  exact figure and update the one constant. (The verified joint-rate path for **both**
  partners applying is separate and unaffected.)
