/* Word Kick service worker. Scoped to /wordkick/ so it never touches the
...
self.addEventListener('activate', e => {
  e.waitUntil(
    caches.keys()
      .then(keys => Promise.all(keys.filter(k => k !== CACHE && k.startsWith('wordkick-')).map(k => caches.delete(k))))
